...
# 2026-04-24
1. UDP는 일대일의 연결관계가 아니므로 하나의 소켓만 필요하다. 따라서 listen(), accept() 함수는 필요가 없다.
   - 클라이언트의 주소 할당은 sendto 함수 호출시 자동으로 할당된다.
...
4. TTL(Time To Live)
   ...
   - TTL은 정수로 표현되며, 라우터를 하나 거칠때마다 1씩 감소
   - TTL이 0이 되면, 해당 패킷은 소멸된다.


5. 배운거 테스트 진행했다.
   [코드](./260424/test_code.c)
